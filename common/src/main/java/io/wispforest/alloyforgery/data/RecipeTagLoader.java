 ...
 */
public class RecipeTagLoader extends SimplePreparableReloadListener<Map<Identifier, List<TagLoader.EntryWithSource>>> {
    ...
    private static final Map<Identifier, List<TagLoader.EntryWithSource>> RAW_TAG_DATA = new LinkedHashMap<>();

    private final DelayedTagGroupLoader<RecipeHolder<Recipe<?>>> tagGroupLoader = new DelayedTagGroupLoader<>("tags/recipe");

    @Override
    protected Map<Identifier, List<TagLoader.EntryWithSource>> prepare(ResourceManager manager, ProfilerFiller profiler) {
        return this.tagGroupLoader.load(manager);
    }

    @Override
    protected void apply(Map<Identifier, List<TagLoader.EntryWithSource>> prepared, ResourceManager manager, ProfilerFiller profiler) {
        RAW_TAG_DATA.clear();
     ...
     */
    public static boolean isWithinTag(boolean isClient, Identifier tag, RecipeHolder<?> entry) {
    ...
    private void resolveEntries(MinecraftServer server) {
        ...
        Map<Identifier, List<RecipeHolder<Recipe<?>>>> map = tagGroupLoader.setGetter(identifier -> {
        ...
        map.forEach((id, recipes) -> RESOLVED_SERVER_ENTRIES.put(id, recipes.stream().map(RecipeHolder::id).map(ResourceKey::identifier).collect(Collectors.toSet())));
